...
if (isset($_REQUEST['seo_queries_import']) && !empty($_REQUEST['seo_queries_text'])) {
	...
	foreach ($queries as $query_line) {
		...
		$meta_title = 'Купить ' . $query_line . ' в Алматы | Контрактные двигатели | Motor Land';
		$meta_description = 'Купить ' . $query_line . ' в Алматы. Контрактные двигатели из Малайзии. Привозные моторы с гарантией. Быстрая доставка по Казахстану.';
		$meta_keywords = mb_strtolower($query_line) . ', купить контрактный двигатель алматы, привозные моторы, двигатель бу малайзия';
		$h1_text = 'Купить ' . $query_line . ' в Алматы';
		
		// Генерируем контент
		$content = '<h2>О ' . htmlspecialchars($query_line, ENT_QUOTES, 'UTF-8') . '</h2>';
		$content .= '<p>Мы предлагаем качественные контрактные двигатели ' . htmlspecialchars($query_line, ENT_QUOTES, 'UTF-8') . ' в Алматы. Все двигатели проходят тщательную проверку перед отправкой и поставляются с гарантией качества.</p>';
		$content .= '<h3>Преимущества покупки у нас:</h3>';
		$content .= '<ul>';
		$content .= '<li>Гарантия качества на все двигатели</li>';
		$content .= '<li>Быстрая доставка по Казахстану</li>';
