...
if ($check->num_rows == 0) {
	...
	// Инициализация страниц с дефолтными значениями (только автосервис, оплата и гарантия)
	$default_pages = [
		['service_page', 'Автосервис', 'Замена Контрактного Двигателя в Алматы', '', 'Автосервис - Замена Двигателей и КПП в Алматы | Моторленд', 'Профессиональная замена и обслуживание контрактных двигателей и КПП в Алматы.', 'замена двигателя алматы, автосервис замена КПП'],
		['pay_page', 'Оплата и Доставка', 'Доставка и Оплата Контрактных Двигателей', '', 'Доставка и Оплата | Контрактные Двигатели и КПП | Моторленд', 'Доставка контрактных двигателей и КПП по Казахстану и СНГ.', 'доставка двигателей алматы, оплата контрактных моторов'],
		['guarantees_page', 'Гарантии', 'Гарантия на Контрактные Двигатели и КПП', '', 'Гарантии на Контрактные Двигатели и КПП | Моторленд', 'Гарантии на контрактные двигатели и КПП в Алматы.', 'гарантия на контрактные двигатели, гарантия на КПП']
	];
...
if (isset($_REQUEST['page_content_save']) || isset($_REQUEST['comand']) && $_REQUEST['comand'] == 'page_content_save') {
	...
	// Разрешенные страницы для редактирования
	$allowed_pages = ['service_page', 'pay_page', 'guarantees_page'];
